...
export class ForgotPasswordComponent {
  ...
  async onSubmit() {
    ...
    if (this.forgotpasswordForm.valid) {
      ...
      try {
        const response = await fetch('http://18.163.194.77/api/user-registration/', {
          method: 'POST',
          headers: {
  ...
  async verifyOTP() {
    ...
    if (
      ...
      try {
        const response = await fetch('http://18.163.194.77/api/verify-forgotpass-otp/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  async sendOTP() {
    ...
    if (
      ...
      try {
        const response = await fetch('http://18.163.194.77/api/send-forgotpass-otp/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
